        ...
        return this.props.wishListItems.map((wishListItem, index) => {
            return (
                <div className={"wishListItem"} key={index}>
                    {this.props.viewMode ? null : <div
                        className={"deleteItem"}
                        onClick={() => this.props.deleteItem(wishListItem)}
                    >x </div>}
                    <h3>{wishListItem.title}</h3>
                    {this.props.viewMode ? <a href={wishListItem.location} target='_blank'>
                        <img src='https://image.flaticon.com/icons/svg/447/447031.svg' style={{ width: '20px' }} /></a> : null}
                </div>
            );
        });
    }
